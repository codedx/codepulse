 ...
 * @author  Illya Kysil
 */
public class UnknownInfoList {

    private List list = new ArrayList();

    /** Creates a new instance of UnknownInfoList */
    public UnknownInfoList() {
    }

    public void add(UnknownInfo unknownInfo) {
        list.add(unknownInfo);
    }

    public void addAll(UnknownInfoList unknownInfoList) {
        for (Iterator iter = unknownInfoList.iterator(); iter.hasNext();) {
            add((UnknownInfo) iter.next());
        }
    }

    public void remove(UnknownInfo unknownInfo) {
        list.remove(unknownInfo);
    }
    ...
    public UnknownInfo[] items() {
        return (UnknownInfo[]) list.toArray(new UnknownInfo[0]);
    }
