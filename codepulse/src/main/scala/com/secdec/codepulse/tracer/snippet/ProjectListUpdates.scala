...
/** A super simple comet actor that calls `$(document).trigger('projectListUpdated')`
  * whenever the given `projectManager` fires an event on its `projectListUpdates` stream.
  */
class ProjectListUpdates(projectManager: ProjectManager) extends PublicCometInit with Observing {
	...
	override def localSetup() = {
		...
		projectManager.projectListUpdates ->> partialUpdate {
			Jq(JsVar("document")) ~> JsFunc("trigger", "projectListUpdated")
		}
	}
}