...
public class VendorInfoList {
    ...
    /** Creates a new instance of VendorInfoList */
    public VendorInfoList() {
    }

    public void add(VendorInfo vendorInfo) {
        list.add(vendorInfo);
    }

    public void addAll(VendorInfoList vendorInfoList) {
        for (Iterator iter = vendorInfoList.iterator(); iter.hasNext();) {
            add((VendorInfo) iter.next());
        }
    }

    public void remove(VendorInfo vendorInfo) {
        list.remove(vendorInfo);
    }
    ...
    public VendorInfo[] items() {
        return (VendorInfo[]) list.toArray(new VendorInfo[0]);
    }
